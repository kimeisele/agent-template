...
Use this repository as a GitHub template to bootstrap a new federation node — complete with authority publishing, peer discovery, A2A Agent Card, and automated workflows.
...
- `.well-known/agent.json` — [A2A-compatible](https://a2aprotocol.ai/) Agent Card
- `docs/authority/capabilities.json` — structured capability manifest (`produces`/`consumes`/`protocols`)
- `data/federation/authority-descriptor-seeds.json` — known peer descriptors (all 7 active nodes)
...
**Phase 2 — Connect:** Agent City zone + registration, Nadi transport, peer discovery
...
Every tier includes the full federation kernel. Federation connection is the default, not opt-in.
...
| Agent City registration | Files a registration issue on [agent-city](https://github.com/kimeisele/agent-city) via `gh` CLI |
| Nadi transport | Creates inbox/outbox, relay config pointing to [steward-federation](https://github.com/kimeisele/steward-federation) hub |
| Nadi relay workflow | GitHub Actions polling the hub every 15 minutes |
| Discovery beacon | `.agent-internet/beacon.json` for control plane discovery |
...
2. Regenerate `.well-known/agent.json` (A2A Agent Card)
...
| `scripts/nadi_relay.py` | Nadi transport relay (generated by wizard) | Auto-generated |
